...
export function AdminDashboard({ products, supply, demand, matches }: Props) {
  ...
  return (
    <main dir="rtl" className="bg-[#f7f3ea] px-4 py-10 text-slate-950">
      <section className="mx-auto max-w-7xl space-y-8">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-sm font-black text-amber-700">لوحة الفريق الداخلية</p>
            <h1 className="mt-2 text-4xl font-black text-emerald-950">مطابقة عروض وطلبات United Fruit</h1>
          </div>
          <button onClick={() => window.location.reload()} className="btn-secondary bg-white text-slate-800 hover:bg-slate-50">
        ...
        <section className="grid gap-5 lg:grid-cols-[1fr_1fr]">
          <form onSubmit={createMatch} className="rounded-lg border border-emerald-900/10 bg-white p-6 shadow-sm">
            <h2 className="text-2xl font-black text-emerald-950">إنشاء مطابقة يدوية</h2>
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              <label className="space-y-2 text-sm font-bold text-slate-700">
                <span>عرض التوفر</span>
                <select className="input bg-white text-slate-950" name="supply_request_id" required>
              ...
              <label className="space-y-2 text-sm font-bold text-slate-700">
                ...
                <select className="input bg-white text-slate-950" name="demand_request_id" required>
              ...
              <label className="space-y-2 text-sm font-bold text-slate-700">
                ...
                <select className="input bg-white text-slate-950" name="status" defaultValue="قيد التنفيذ">
          ...
          <form onSubmit={updatePrice} className="rounded-lg border border-emerald-900/10 bg-white p-6 shadow-sm">
            <h2 className="text-2xl font-black text-emerald-950">تحديث أسعار المنتجات</h2>
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              <label className="space-y-2 text-sm font-bold text-slate-700 sm:col-span-2">
                <span>المنتج</span>
                <select className="input bg-white text-slate-950" name="product_id" required>
        ...
        <Panel title="سجل الصفقات المحتملة">
          <div className="grid gap-3">
            {matches.length ? (
              matches.map((match) => (
                <article key={match.match_id} className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                  <div className="flex flex-wrap justify-between gap-3">
                    <p className="font-black text-emerald-950">{match.status}</p>
              ...
              <p className="rounded-lg bg-slate-50 p-4 text-sm font-bold text-slate-500">لا توجد مطابقات بعد.</p>
            )}
          </div>
        </Panel>
      </section>
...
function Field({ name, label, type = 'text' }: { name: string; label: string; type?: string }) {
  return (
    <label className="space-y-2 text-sm font-bold text-slate-700">
      <span>{label}</span>
      <input className="input bg-white text-slate-950" name={name} type={type} />
    </label>
  );
}
...
function RequestList({ rows, kind }: { rows: any[]; kind: 'supply' | 'demand' }) {
  if (!rows.length) {
    return <p className="rounded-lg bg-slate-50 p-4 text-sm font-bold text-slate-500">لا توجد سجلات بعد.</p>;
  }
      ...
      {rows.map((row) => (
        <article key={row.supply_request_id || row.demand_request_id} className="rounded-lg border border-slate-200 bg-slate-50 p-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <p className="font-black text-emerald-950">{row.uf_products?.name_ar || row.product_id}</p>
