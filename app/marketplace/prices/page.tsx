...
export default async function PriceBoardPage() {
  ...
  return (
    <main dir="rtl" className="bg-[#fbfdf8] px-4 py-12 text-[#173a2b]">
      <section className="mx-auto max-w-7xl">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-sm font-black text-amber-700">United Fruit Company</p>
            <h1 className="mt-2 text-4xl font-black text-[#173a2b]">لوحة أسعار المحاصيل</h1>
          </div>
            ...
            <tbody>
              {products.map((product) => (
                <tr key={product.product_id} className="border-t border-slate-100">
                  <td className="px-5 py-4 font-black text-[#173a2b]">{product.name_ar}</td>
