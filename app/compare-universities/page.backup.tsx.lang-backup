...
function languageHref(
  ...
  if (compareQuery) {
    params.set('compare', compareQuery);
  }

  const query = params.toString();

  return `/compare-universities${query ? `?${query}` : ''}`;
}
  ...
  return (
    <main dir={t.dir} className="mx-auto max-w-7xl space-y-8 px-4 py-12">
      <div className="flex justify-end gap-3">
        <Link
          href={languageHref('ar', selectedCity, compareQuery)}
        ...
        <Link
          href={languageHref('en', selectedCity, compareQuery)}
          className={`rounded-full border px-5 py-2 font-bold ${
            locale === 'en'
              ? 'border-slate-950 bg-slate-950 text-white'
              : 'border-slate-200 bg-white text-slate-900 hover:bg-slate-50'
